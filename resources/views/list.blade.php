<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('List of Test Drive Registration') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-[-70px]">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mx-[270px] text-xl">
                <div class="p-6 text-gray-900 dark:text-gray-100 text-center ">
                    Total of {{count($list)}} application(s)
                </div>
            </div>
        </div>
    </div>
    ...
    @if (count($list)>0)
        <div class="overflow-x-auto bg-gray-700 mx-[100px] mb-10 ">
                <table class="table table-lg">
                    ...
                    <tr onclick="window.location.href = '{{ route('details', ['id' => $list->appointment_id]) }}';" style="cursor: pointer;">
                        ...
                        <td>{{$list->purchase_status?'Ongoing':'No'}}</td>
